...
client = Client("https://api.example.com";
                auth = BearerToken(ENV["ENTSOE_TOKEN"]))
```

`resolve_credentials` picks up env vars (`ENTSOE_TOKEN`,
`ENTSOE_API_KEY`, `ENTSOE_USERNAME` / `ENTSOE_PASSWORD`)
or falls back to `~/.config/entsoe/credentials.toml`:
...
- [ ] Replace the placeholder `ENV["ENTSOE_TOKEN"]` references
